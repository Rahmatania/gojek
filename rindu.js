...
$(document).ready(function() {
	...
	iDeviceNotOS4 = (navigator.userAgent.match(/iphone|ipod|ipad/i) && !navigator.
		userAgent.match(/OS 5/i)) || false,
	badlE = $('html').prop('class').match(/ie(6|7|8)/)|| 
	false;
...
win.bind('load', function(){
	...
	win.bind('scroll', function(event){
		...
		timeout = setTimeout(function(){
			...
			pos = window.pageYOffset || document.documentElement.scroll||Top || document.body.scrollTop;
...
//the function is called when the hash 
changes 
function hashchange(){
...
function goTo(hash, changehash){
	...
	hash = hash.replace(/!\//,'');
		...
		if(changehash !== false){
			var | = location;
			location.href = (l.protocol+'//'+l.host+|.pathname+'#!/'+hash.substr(1));
		}
	...
	function acctiveNav(pos){
		var offset = 100,
		current, next, parent, isSub, hasSub;
		win.unbind('hashchange', hashchange);
		for(var i=sectionscount;i>0;i--){
			if(section[i-1].pos <= pos+offset){
